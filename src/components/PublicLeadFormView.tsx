...
const ENERGY_BILL_ACCEPT = 'application/pdf,image/jpeg,image/png';
const ENERGY_BILL_MIME_TYPES = new Set(['application/pdf', 'image/jpeg', 'image/png']);
...
export const PublicLeadFormView: React.FC<PublicLeadFormViewProps> = ({ formToken }) => {
  ...
  const energyBillInputRef = useRef<HTMLInputElement>(null);
  ...
  const handleEnergyBillChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0] ?? null;
    if (!file) return;
    if (!ENERGY_BILL_MIME_TYPES.has(file.type)) {
      event.target.value = '';
      setEnergyBillFile(null);
      setError('Envie a conta de luz em PDF, JPG ou PNG.');
      return;
    }
    if (file.size > MAX_ENERGY_BILL_BYTES) {
      event.target.value = '';
      setEnergyBillFile(null);
      setError('A conta de luz deve ter no máximo 8 MB.');
      return;
    }
    setEnergyBillFile(file);
    setError('');
  };

  const removeEnergyBill = () => {
    setEnergyBillFile(null);
    if (energyBillInputRef.current) energyBillInputRef.current.value = '';
    setError('');
  };
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (queryContext.widget && queryContext.siteOrigin && window.parent !== window) {
      ...
      window.parent.postMessage(
        { type: 'sol-amigo:submit', formToken, payload, energyBillFile },
        hostOrigin
      );
      return;
    }
    ...
    if (energyBillFile) {
      multipartBody.append('energyBill', energyBillFile, energyBillFile.name);
    }
    ...
    if (invokeError || data.error) {
      setError(typeof data.error === 'string' ? data.error : 'Não foi possível enviar seus dados. Tente novamente.');
      return;
    }
              ...
              {step === 1 ? (
                ...
                <>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <div data-form-panel className="sm:col-span-2 rounded-xl border border-dashed p-3" style={{ borderColor: resolvedTheme.inputBorder, backgroundColor: resolvedTheme.consentBackground }}>
                      <input
                        ref={energyBillInputRef}
                        id={`energy-bill-${formToken}`}
                        type="file"
                        accept={ENERGY_BILL_ACCEPT}
                        onChange={handleEnergyBillChange}
                        className="sr-only"
                      />
                      {energyBillFile ? (
                        <div className="flex items-center gap-3">
                          <div data-form-compact className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl" style={{ backgroundColor: resolvedTheme.cardBackground, color: resolvedTheme.progressActive }}>
                            <FileText className="h-5 w-5" />
                          </div>
                          <div className="min-w-0 flex-1">
                            <p className="truncate text-xs font-extrabold">{energyBillFile.name}</p>
                            <p className="mt-0.5 text-[11px]" style={{ color: resolvedTheme.mutedText }}>
                              {(energyBillFile.size / (1024 * 1024)).toFixed(2).replace('.', ',')} MB · pronto para enviar
                            </p>
                          </div>
                          <button
                            type="button"
                            onClick={removeEnergyBill}
                            aria-label="Remover conta de luz"
                            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border transition-colors"
                            style={{ borderColor: resolvedTheme.inputBorder, color: resolvedTheme.errorAccent }}
                          >
                            <Trash2 className="h-4 w-4" />
                          </button>
                        </div>
                      ) : (
                        <label htmlFor={`energy-bill-${formToken}`} className="flex cursor-pointer items-center gap-3">
                          <div data-form-compact className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl" style={{ backgroundColor: resolvedTheme.cardBackground, color: resolvedTheme.progressActive }}>
                            <UploadCloud className="h-5 w-5" />
                          </div>
                          <div className="min-w-0">
                            <p className="text-xs font-extrabold">Enviar conta de luz <span className="font-medium" style={{ color: resolvedTheme.mutedText }}>(opcional)</span></p>
                            <p className="mt-0.5 text-[11px]" style={{ color: resolvedTheme.mutedText }}>PDF, JPG ou PNG · até 8 MB</p>
                          </div>
                        </label>
                      )}
                    </div>
                  ...
                  <label data-form-panel className="mt-5 flex items-start gap-3 rounded-xl p-3" style={{ backgroundColor: resolvedTheme.consentBackground }}>
                    ...
                    <span className="text-xs leading-5" style={{ color: resolvedTheme.mutedText }}>Autorizo o contato da equipe para atender esta solicitação e concordo com o tratamento dos dados e documentos enviados para essa finalidade.{config.privacyUrl && <> Consulte a <a href={config.privacyUrl} target="_blank" rel="noreferrer" className="font-bold underline">política de privacidade</a>.</>}</span>
                  </label>
