...
const money = (value?: number) =>
  new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(Number.isFinite(Number(value)) ? Number(value) : 0);
...
const loadSafeKits = (): SolarKit[] => {
  try {
    ...
    return Array.isArray(loaded) ? loaded.filter((kit): kit is SolarKit => Boolean(kit && typeof kit === 'object')) : [];
  } catch (error) {
...
export const KitsView: React.FC<KitsViewProps> = ({ theme, onShowToast }) => {
  ...
  const handleCreate = () => {
    if (!name.trim() || powerKWp <= 0 || modulePowerW <= 0) {
      onShowToast?.('Informe o nome, a potência do kit e a potência do módulo.');
      return;
    }
    const created = addCustomKit({
      name, systemType, powerKWp,
      minPowerKWp: powerKWp * 0.9,
      maxPowerKWp: powerKWp * 1.1,
      modulePowerW,
      moduleCount: Math.ceil((powerKWp * 1000) / modulePowerW),
      moduleModel, inverterModel, equipmentCost, suggestedPrice,
    });
    ...
    <section className="space-y-6" id="kits-page">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={restore} className="inline-flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-semibold" style={{ borderColor: theme.border }}><RotateCcw className="h-4 w-4" /> Restaurar padrão</button>
          <button onClick={() => setShowForm((value) => !value)} className="inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold" style={{ backgroundColor: theme.secondary, color: '#fff' }}><Plus className="h-4 w-4" /> Novo kit</button>
        </div>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-3">
        {[['Total', kits.length], ['On-Grid', kits.filter((k) => k.systemType === 'On-Grid').length], ['Híbridos', kits.filter((k) => k.systemType === 'Híbrido').length]].map(([label, value]) => (
          <div key={String(label)} className="rounded-2xl border p-4" style={{ borderColor: theme.border, backgroundColor: theme.primary }}><div className="text-xs font-semibold uppercase tracking-wider opacity-60">{label}</div><div className="mt-1 text-2xl font-bold">{value}</div></div>
        ))}
      </div>
      ...
      {showForm && (
        <div className="rounded-2xl border p-5" style={{ borderColor: theme.border, backgroundColor: theme.primary }}>
          ...
          <div className="mt-4 flex justify-end gap-2"><button onClick={() => setShowForm(false)} className="rounded-xl border px-4 py-2 text-sm font-semibold" style={{ borderColor: theme.border }}>Cancelar</button><button onClick={handleCreate} className="rounded-xl px-4 py-2 text-sm font-bold" style={{ backgroundColor: theme.accent, color: '#0E2337' }}>Salvar kit</button></div>
        </div>
      )}

      <div className="flex flex-col gap-3 md:flex-row">
        <label className="relative flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 opacity-50" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar por nome, SKU, módulo ou inversor..." className="w-full rounded-xl border py-2.5 pl-10 pr-3" style={inputStyle} /></label>
      ...
      {visibleKits.length === 0 ? (
        <div className="rounded-2xl border p-10 text-center" style={{ borderColor: theme.border }}><Package className="mx-auto mb-3 h-8 w-8 opacity-40" /><p className="font-semibold">Nenhum kit encontrado.</p><button onClick={restore} className="mt-4 rounded-xl px-4 py-2 text-sm font-bold" style={{ backgroundColor: theme.secondary, color: '#fff' }}>Carregar catálogo padrão</button></div>
      ) : (
        <div className="grid gap-4 xl:grid-cols-2">
          {visibleKits.map((kit, index) => {
            ...
            return (
              <article key={safeId} className="rounded-2xl border p-5" style={{ borderColor: theme.border, backgroundColor: theme.primary }}>
                <div className="flex items-start justify-between gap-4"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full px-2.5 py-1 text-[11px] font-bold" style={{ backgroundColor: theme.secondary, color: '#fff' }}>{kit.systemType || 'On-Grid'}</span><span className="text-xs opacity-50">{kit.sku || 'Sem SKU'}</span></div><h3 className="mt-2 text-base font-bold">{kit.name || 'Kit sem nome'}</h3></div><button onClick={() => removeKit(safeId)} title="Excluir kit" className="rounded-lg p-2 opacity-60 hover:opacity-100"><Trash2 className="h-4 w-4" /></button></div>
                <div className="mt-4 grid grid-cols-2 gap-3 text-sm md:grid-cols-4"><div><div className="text-xs opacity-50">Potência</div><div className="font-bold">{power.toFixed(2)} kWp</div></div><div><div className="text-xs opacity-50">Módulos</div><div className="font-bold">{kit.moduleCount || '—'} × {kit.modulePowerW || '—'} W</div></div><div><div className="text-xs opacity-50">Custo equip.</div><div className="font-bold">{money(kit.equipmentCost)}</div></div><div><div className="text-xs opacity-50">Preço sugerido</div><div className="font-bold">{money(kit.suggestedPrice)}</div></div></div>
                <div className="mt-4 space-y-2 border-t pt-4 text-sm" style={{ borderColor: theme.border }}><div className="flex gap-2"><Zap className="mt-0.5 h-4 w-4 shrink-0 opacity-60" /><span>{kit.inverterModel || 'Inversor não informado'}</span></div><div className="flex gap-2"><Package className="mt-0.5 h-4 w-4 shrink-0 opacity-60" /><span>{kit.moduleModel || 'Módulo não informado'}</span></div>{kit.systemType === 'Híbrido' && <div className="flex gap-2"><Battery className="mt-0.5 h-4 w-4 shrink-0 opacity-60" /><span>{kit.batteryModel || 'Bateria não informada'} {kit.batteryCapacityKWh ? `• ${kit.batteryCapacityKWh} kWh` : ''}</span></div>}</div>
                <div className="mt-4 flex items-center justify-between gap-3"><span className="text-xs opacity-60">Faixa: {minPower.toFixed(1)}–{maxPower.toFixed(1)} kWp</span><button onClick={() => toggleActive(safeId)} className="rounded-full border px-3 py-1.5 text-xs font-bold" style={{ borderColor: kit.active !== false ? theme.accent : theme.border, color: kit.active !== false ? theme.accent : theme.text }}>{kit.active !== false ? 'Ativo' : 'Inativo'}</button></div>
              </article>
            );
          })}
        </div>
