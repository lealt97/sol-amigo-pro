...
interface LoginViewProps {
  onLogin: (credentials: { email: string; password: string; remember: boolean }) => Promise<string | null>;
  onForgotPassword: (email: string) => Promise<string>;
  onOpenRegister: () => void;
}

export const LoginView: React.FC<LoginViewProps> = ({ onLogin, onForgotPassword, onOpenRegister }) => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    const loginError = await onLogin({ email: normalizedEmail, password, remember });
      ...
      <div className="mx-auto grid min-h-[calc(100vh-40px)] w-full max-w-[1380px] overflow-hidden rounded-[28px] border border-[#D6E0E8] bg-white shadow-[0_28px_80px_rgba(14,35,55,0.16)] lg:grid-cols-[46%_54%]">
        <section className="flex min-h-[720px] items-center bg-white px-7 py-10 sm:px-12 lg:px-16 xl:px-20">
          <div className="mx-auto w-full max-w-[470px]">
            <BrandLogo orientation="horizontal" backgroundColor="#FFFFFF" className="mb-14 h-auto w-[210px] max-w-full object-contain object-left" />

            <div className="mb-9">
              <span className="mb-3 inline-flex rounded-full bg-[#B4BF8A]/20 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.16em] text-[#183956]">Acesso ao sistema</span>
              <h1 className="text-4xl font-extrabold tracking-[-0.035em] text-[#0E2337] sm:text-[42px]">Bem-vindo!</h1>
              <p className="mt-3 max-w-[410px] text-[15px] leading-6 text-[#607386]">Acesse sua conta para gerenciar clientes, propostas e projetos de energia solar.</p>
            </div>

            <form onSubmit={handleSubmit} className="space-y-5" noValidate>
              <div>
                <label htmlFor="login-email" className="mb-2 block text-sm font-bold text-[#183956]">E-mail</label>
                <div className="relative">
                  <Mail className="pointer-events-none absolute left-4 top-1/2 h-[18px] w-[18px] -translate-y-1/2 text-[#7D8E9E]" />
                  <input id="login-email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="seu@email.com" autoComplete="email" className="h-[52px] w-full rounded-xl border border-[#CFD9E2] bg-white pl-12 pr-4 text-[14px] text-[#0E2337] outline-none transition focus:border-[#0076DD] focus:ring-4 focus:ring-[#64B0F3]/15" />
                </div>
              </div>

              <div>
                <label htmlFor="login-password" className="mb-2 block text-sm font-bold text-[#183956]">Senha</label>
                <div className="relative">
                  <LockKeyhole className="pointer-events-none absolute left-4 top-1/2 h-[18px] w-[18px] -translate-y-1/2 text-[#7D8E9E]" />
                  <input id="login-password" type={showPassword ? 'text' : 'password'} value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Digite sua senha" autoComplete="current-password" className="h-[52px] w-full rounded-xl border border-[#CFD9E2] bg-white pl-12 pr-12 text-[14px] text-[#0E2337] outline-none transition focus:border-[#0076DD] focus:ring-4 focus:ring-[#64B0F3]/15" />
                  <button type="button" onClick={() => setShowPassword((current) => !current)} className="absolute right-3 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-lg text-[#718496] transition hover:bg-[#F1F5F8] hover:text-[#183956]" aria-label={showPassword ? 'Ocultar senha' : 'Mostrar senha'}>
              ...
              <div className="flex items-center justify-between gap-4 text-[13px]">
                <label className="flex cursor-pointer items-center gap-2.5 font-medium text-[#41586C]">
                  <input type="checkbox" checked={remember} onChange={(event) => setRemember(event.target.checked)} className="h-4 w-4 accent-[#0076DD]" />
                  Lembrar-me
                </label>
                <button type="button" disabled={loading} onClick={handleForgotPassword} className="font-semibold text-[#0076DD] transition hover:text-[#005EAF]">Esqueci minha senha</button>
              </div>
              ...
              <button type="submit" disabled={loading} className="flex h-[52px] w-full items-center justify-center rounded-xl bg-[#183956] px-5 text-sm font-bold text-white shadow-[0_10px_24px_rgba(24,57,86,0.20)] transition hover:bg-[#0E2337] active:translate-y-px disabled:opacity-60">
            ...
            <div className="mt-6 text-center text-[13px] text-[#607386]">
              ...
              <button type="button" onClick={onOpenRegister} className="font-bold text-[#0076DD] transition hover:text-[#005EAF]">Criar conta</button>
            </div>
            ...
            <p className="mt-10 text-center text-[12px] text-[#8797A5]">© 2026 Sol Amigo Pro</p>
          </div>
        </section>
